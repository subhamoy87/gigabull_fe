const BACKEND_URL = 'https://gigabull-be.vercel.app'; // Your Vercel backend URL

//export const CONTACT_API_URL = 'https://contact-forms.dhigrowth.com/api/v1/contact/gigabull/contact';
export const CONTACT_API_URL = `${BACKEND_URL}/api/contact`;
export const PING_API_URL = `${BACKEND_URL}/api/ping`;
export const ENQUIRY_API_URL = `${BACKEND_URL}/api/enquiries`;
//export const ENQUIRY_API_URL = 'https://contact-forms.dhigrowth.com/api/v1/contact/gigabull/enquiry';
export const ADMIN_LOGIN_API_URL = `${BACKEND_URL}/api/admin/login`;
export const ADMIN_CHANGE_PASSWORD_API_URL = `${BACKEND_URL}/api/admin/change-password`;
export const PRODUCTS_API_URL = `${BACKEND_URL}/api/products`;
export const SAVE_PRODUCTS_API_URL = `${BACKEND_URL}/api/save-products-data`;
export const DOCUMENTS_API_URL = `${BACKEND_URL}/api/documents`;
export const SAVE_DOCUMENTS_API_URL = `${BACKEND_URL}/api/save-documents`;


//export const CONTACT_API_URL = 'https://gigabull-server.onrender.com/api/contact';
//export const PING_API_URL = 'https://gigabull-server.onrender.com/api/ping';
// export const ENQUIRY_API_URL = 'https://gigabull-server.onrender.com/api/enquiries';
//export const ENQUIRY_API_URL = 'http://localhost:8080/api/enquiries';